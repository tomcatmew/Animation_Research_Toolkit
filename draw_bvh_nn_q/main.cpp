...
void q_normalize(std::vector<double>& aValueQ) {
	for (int i = 0; i < aValueQ.size(); i++)
	{
		if (i == 0 || i == 1 || i == 2)
			continue;
		else if ((i + 1) % 4 == 0)
		{
			double n = sqrt(aValueQ[i] * aValueQ[i] + aValueQ[i+1] * aValueQ[i+1] + aValueQ[i+2] * aValueQ[i+2] + aValueQ[i+3] * aValueQ[i+3]);
			aValueQ[i] /= n;
			aValueQ[i+1] /= n;
			aValueQ[i+2] /= n;
			aValueQ[i+3] /= n;
...
//[2]load BVH and load Q file main program --------------------------------------[2]
	...
	const std::string& path_bvh)
{
	...
	std::cout << "the ALL value numer : " << aValueRotTransBone.size() << std::endl;
	for (int iframe = 0; iframe < nframe; ++iframe) {
		std::getline(fin, line);
		//line = rig_v3q::MyReplace(line, '\t', ' ');   Prof.Umetani replace tab to space
		if (line[line.size() - 1] == '\n') line.erase(line.size() - 1); // remove the newline code
...
int main(void)
{
	...
	try {
		...
		module = torch::jit::load("models/q_traced_v3.pt");
	}
	...
	std::fstream in("models/q_start.bvh");
	...
	if (!glfwInit())
		exit(EXIT_FAILURE);
	window = glfwCreateWindow(1080, 880, "bhv_nn_q", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		exit(EXIT_FAILURE);
	...
	std::string path_bvh = "models/LocomotionFlat01_000.bvh";
	//std::string path_bvh = "10_01.bvh";
	std::vector<double> aValueQ;
	int qframe = 8169;
	std::string path_q = "models/output_quaternionx.bvh";
	...
	for (int k = 0; k < 500; k++)
	{
		std::cout << "Q value " << k << " : " << aValueQ[k] << std::endl;
	}
	...
	std::cout << "Quanternion size:" << aValueQ.size() << std::endl;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		else
		{
			...
			SetPose_BioVisionHierarchy_Quat(aBone, aChannelRotTransBone, aValueQ.data() + iqframe * nch);
			...
			iqframe = (iqframe + 1) % nframe;  // repeat playing this character animation 
			std::cout << " Frame : " << iqframe << std::endl;
		}
